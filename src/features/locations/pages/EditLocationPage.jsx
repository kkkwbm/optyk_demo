...
import { fetchLocationById, updateLocation } from '../locationsSlice';
...
function EditLocationPage() {
  ...
  useEffect(() => {
    const loadLocation = async () => {
      try {
        const location = await dispatch(fetchLocationById(id)).unwrap();
        reset({
          name: location.name || '',
          type: location.type || LOCATION_TYPES.STORE,
        ...
        toast.error(error || 'Nie udało się załadować lokalizacji');
        navigate('/locations');
      } finally {
        setLoading(false);
      }
  ...
  const onSubmit = async (data) => {
    try {
      await dispatch(updateLocation({ id, data })).unwrap();
  ...
  return (
    <Container maxWidth="lg">
      <PageHeader
        ...
        subtitle="Aktualizuj informacje o lokalizacji"
        breadcrumbs={[
          { label: 'Pulpit', to: '/' },
